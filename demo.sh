...
start_backend() {
    ...
    # Start in background (0.0.0.0 allows LAN access)
    nohup uvicorn app.main:app --reload --host 0.0.0.0 --port 8000 > /tmp/nodus-backend.log 2>&1 &
    echo $! > /tmp/nodus-backend.pid
    ...
    print_status error "Backend failed to start. Check /tmp/nodus-backend.log"
    return 1
}
...
start_frontend() {
    ...
    # Start in background (0.0.0.0 allows LAN access)
    nohup pnpm dev -H 0.0.0.0 > /tmp/nodus-frontend.log 2>&1 &
    echo $! > /tmp/nodus-frontend.pid
    ...
    print_status error "Frontend failed to start. Check /tmp/nodus-frontend.log"
    return 1
}
...
stop_servers() {
    ...
    # Stop backend
    if [ -f /tmp/nodus-backend.pid ]; then
        PID=$(cat /tmp/nodus-backend.pid)
        if kill -0 $PID 2>/dev/null; then
            kill $PID 2>/dev/null || true
            print_status success "Backend stopped"
        fi
        rm -f /tmp/nodus-backend.pid
    fi
    ...
    # Stop frontend
    if [ -f /tmp/nodus-frontend.pid ]; then
        PID=$(cat /tmp/nodus-frontend.pid)
        if kill -0 $PID 2>/dev/null; then
            kill $PID 2>/dev/null || true
            print_status success "Frontend stopped"
        fi
        rm -f /tmp/nodus-frontend.pid
    fi
...
reset_demo() {
    ...
    rm -f "$BACKEND_DIR/data/nodus.db"
