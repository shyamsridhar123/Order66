...
4. A clear task description""",
            ...
            intent = {
                "primary_intent": intent.get("primary_intent", "question"),
                "required_agents": intent.get("required_agents", ["memory", "advisor"]),
                "key_entities": intent.get("key_entities", []),
                "task_description": intent.get("task_description", message_content)
            }
        except Exception as e:
            # Fallback: use simple intent detection for small models
            message_lower = message_content.lower()
            if "proposal" in message_lower:
                intent = {
                    "primary_intent": "proposal",
                    "required_agents": ["strategist", "researcher", "analyst", "memory", "scribe"],
                    "key_entities": [],
                    "task_description": message_content
                }
            elif "research" in message_lower or "briefing" in message_lower:
                intent = {
                    "primary_intent": "research",
                    "required_agents": ["researcher", "memory", "advisor"],
                    "key_entities": [],
                    "task_description": message_content
                }
            elif "analy" in message_lower:
                intent = {
                    "primary_intent": "analysis",
                    "required_agents": ["analyst", "memory"],
                    "key_entities": [],
                    "task_description": message_content
                }
            else:
                intent = {
                    ...
                    "required_agents": ["memory", "advisor"],
                    "key_entities": [],
                    "task_description": message_content
                }
        ...
        if intent["primary_intent"] == "proposal":
            ...
            proposal_agents = ["strategist", "researcher", "analyst", "memory", "advisor", "scribe"]
            agents_to_run = list(dict.fromkeys(intent["required_agents"] + proposal_agents))[:7]
        elif intent["primary_intent"] == "research":
            # Research should include all relevant agents
            research_agents = ["researcher", "memory", "analyst", "advisor", "scribe"]
            agents_to_run = list(dict.fromkeys(intent["required_agents"] + research_agents))[:6]
        elif intent["primary_intent"] == "analysis":
            # Analysis includes analyst, memory, and advisor
            analysis_agents = ["analyst", "memory", "advisor", "scribe"]
            agents_to_run = list(dict.fromkeys(intent["required_agents"] + analysis_agents))[:5]
        else:
            # For simple queries, still show some agent activity
            default_agents = ["memory", "advisor"]
            agents_to_run = list(dict.fromkeys(intent["required_agents"] + default_agents))[:4]
...
async def _execute_agent(
    ...
    try:
        ...
        # Pass ws_manager and conversation_id for tool call notifications
        if agent_name == "strategist":
            result, tokens_used = await run_strategist(task, context)
        elif agent_name == "researcher":
            result, tokens_used = await run_researcher(task, context, ws_manager, conversation_id)
        elif agent_name == "analyst":
            result, tokens_used = await run_analyst(task, context, ws_manager, conversation_id)
        elif agent_name == "scribe":
            result, tokens_used = await run_scribe(task, context, ws_manager, conversation_id)
        elif agent_name == "advisor":
            result, tokens_used = await run_advisor(task, context, ws_manager, conversation_id)
        elif agent_name == "memory":
            result, tokens_used = await run_memory(task, context, ws_manager, conversation_id)
        else:
