...
async def submit_feedback(
    ...
    rating = feedback.get("rating")  # 1 or -1
    comment = feedback.get("comment")
    
    # Store in debug_logs if debug mode was on, or just log
    # For now, we'll update the message metadata
    if message_id:
        result = await db.execute(
    ...
    # If no message found, just acknowledge
    return {
        "success": True,
